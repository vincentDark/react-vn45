            ...
            <div styleName="row full colum-holder" ref="scroll">
              <div styleName="inside">
                {new Array(+columnNum).fill('').map((item, index) => {
                  ...
                  return (
                    <div
                      styleName={`gamecolum border-right ${active}`}
                      key={`inside${index}`}
                      onClick={this.chooseCloum(index)}>
                      <div styleName="subtitle2">柱數 {index + 1}</div>
                      {ColumBalls[index] !== undefined &&
                        ColumBalls[index].map((ball, temp) => {
                          return <p key={`C${index}ball${ball}`}>{ball}</p>;
                        })}
                    </div>
                  );
                })}
...
export default compose(withDispatch, withStore('BallsState'))(
  withRouter(withStyle()(CPonFrame))
);
